...
async function analyze(imgPath, name) {
  try {
    ...
    console.log(`\nTop 5 colors in ${name} (${info.width}x${info.height}):`);
    sorted.slice(0, 5).forEach(([color, count]) => {
...
async function run() {
  await analyze(lightImgPath, 'Light Logo');
  await analyze(darkImgPath, 'Dark Logo');
}
